import Image from "next/image";
import dataJson from "@/data/data.json";
import clubsJson from "@/data/clubs.json";

export default function EducationClubs() {
  return (
    <section
      id="education-clubs"
      className="scroll-mt-24 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 fade-up-element"
    >
      {/* ─── SECTION 06: Education Timeline ─── */}
      <div className="flex flex-col gap-6">
        <div className="border-b border-border-custom pb-4 flex items-center justify-between">
          <h2 className="font-serif text-4xl md:text-5xl tracking-tight text-foreground">
            06 / Education
          </h2>
          <span className="text-xs font-mono text-muted">✦ Academic Journey</span>
        </div>

        {/* Vertical Timeline Spine */}
        <div className="relative border-l-2 border-border-custom ml-3 pl-6 flex flex-col gap-10 py-2">
          {dataJson.education.map((edu, index) => (
            <div
              key={index}
              className="relative group fade-up-item"
              style={{ transitionDelay: `${(index + 1) * 100}ms` }}
            >
              {/* Timeline Node Dot */}
              <div className="absolute -left-[31px] top-1 w-3.5 h-3.5 rounded-full bg-background border-2 border-accent group-hover:bg-accent group-hover:scale-125 transition-all duration-300 shadow-xs" />

              <div className="flex items-start gap-4">
                {/* School Logo Box */}
                <div className="w-10 h-10 border border-border-custom rounded-md bg-card p-1.5 shrink-0 relative overflow-hidden flex items-center justify-center shadow-xs group-hover:border-accent/40 transition-colors">
                  <Image
                    src={edu.logoUrl}
                    alt={edu.school}
                    width={32}
                    height={32}
                    className="object-contain"
                  />
                </div>

                <div className="space-y-1 font-mono flex-1">
                  <h3 className="font-serif text-lg font-bold text-foreground leading-tight hover:text-accent transition-colors">
                    <a
                      href={edu.href}
                      target="_blank"
                      rel="noopener noreferrer"
                      className="inline-flex items-center gap-1.5"
                    >
                      <span>{edu.school}</span>
                      <span className="text-xs text-muted group-hover:text-accent transition-colors">↗</span>
                    </a>
                  </h3>
                  <p className="text-xs text-muted">{edu.degree}</p>
                  <p className="text-[10px] text-accent font-semibold pt-0.5">
                    {edu.start} — {edu.end}
                  </p>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>

      {/* ─── SECTION 07: Leadership Timeline ─── */}
      <div className="flex flex-col gap-6">
        <div className="border-b border-border-custom pb-4 flex items-center justify-between">
          <h2 className="font-serif text-4xl md:text-5xl tracking-tight text-foreground">
            07 / Leadership
          </h2>
          <span className="text-xs font-mono text-muted">✦ Student Initiatives</span>
        </div>

        {/* Vertical Timeline Spine */}
        <div className="relative border-l-2 border-border-custom ml-3 pl-6 flex flex-col gap-10 py-2">
          {clubsJson.map((club, index) => (
            <div
              key={index}
              className="relative group fade-up-item"
              style={{ transitionDelay: `${(index + 1) * 100}ms` }}
            >
              {/* Timeline Node Dot */}
              <div className="absolute -left-[31px] top-1 w-3.5 h-3.5 rounded-full bg-background border-2 border-accent group-hover:bg-accent group-hover:scale-125 transition-all duration-300 shadow-xs" />

              <div className="flex items-start gap-4">
                {/* Organization Logo Box */}
                <div className="w-10 h-10 border border-border-custom rounded-md bg-card p-1.5 shrink-0 relative overflow-hidden flex items-center justify-center shadow-xs group-hover:border-accent/40 transition-colors">
                  <Image
                    src={club.image}
                    alt={club.organization}
                    width={32}
                    height={32}
                    className="object-contain"
                  />
                </div>

                <div className="space-y-2 flex-1 font-mono">
                  <h3 className="font-serif text-lg font-bold text-foreground leading-tight">
                    {club.organization}
                  </h3>

                  <div className="space-y-2.5">
                    {club.post.map((p, pIdx) => (
                      <div
                        key={pIdx}
                        className="text-xs border-l-2 border-accent/60 pl-3 py-0.5 space-y-1"
                      >
                        <div className="flex justify-between items-baseline gap-2 flex-wrap">
                          <span className="font-semibold text-foreground">
                            {p.title}
                          </span>
                          <span className="text-[10px] text-accent font-semibold">
                            {p.dates}
                          </span>
                        </div>
                        <p className="text-muted leading-relaxed text-[11px]">
                          {p.description}
                        </p>
                      </div>
                    ))}
                  </div>

                  {club.links && club.links.length > 0 && (
                    <div className="flex gap-3 text-[10px] font-semibold pt-1">
                      {club.links.map((link, lIdx) => (
                        <a
                          key={lIdx}
                          href={link.href}
                          target="_blank"
                          rel="noopener noreferrer"
                          className="text-accent hover:underline"
                        >
                          [{link.title} ↗]
                        </a>
                      ))}
                    </div>
                  )}
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
